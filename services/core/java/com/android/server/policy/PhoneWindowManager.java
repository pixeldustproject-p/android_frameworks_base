...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    class SettingsObserver extends ContentObserver {
	    ...
	    resolver.registerContentObserver(Settings.System.getUriFor(
            ...
            resolver.registerContentObserver(Settings.Secure.getUriFor(
                    ...
                    UserHandle.USER_ALL);
	    resolver.registerContentObserver(Settings.Secure.getUriFor(
                    Settings.Secure.LONG_PRESS_KILL_DELAY), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System.getUriFor(
    ...
    Runnable mBackLongPress = new Runnable() {
        public void run() {
            if (PixeldustUtils.killForegroundApplication(mContext)) {
                performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
    ...
    private void readConfigurationDependentBehaviors() {
        ...
        final boolean hasHome = mNavbarVisible || (mDeviceHardwareKeys & NavbarUtilities.KEY_MASK_HOME) != 0;
        final boolean hasAppSwitch = mNavbarVisible || (mDeviceHardwareKeys & NavbarUtilities.KEY_MASK_APP_SWITCH) != 0;
        final boolean hasBack = mNavbarVisible || (mDeviceHardwareKeys & NavbarUtilities.KEY_MASK_BACK) != 0;
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            // longpress back kill delay
            mBackKillTimeout = Settings.Secure.getIntForUser(resolver,
                    Settings.Secure.LONG_PRESS_KILL_DELAY, 1000, UserHandle.USER_CURRENT);
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        // Custom longpress back key to kill app
        if (keyCode == KeyEvent.KEYCODE_BACK && !down) {
            mHandler.removeCallbacks(mBackLongPress);
        }

        // Custom event handling for supported key codes.
        if (!PixeldustUtils.isInLockTaskMode() && NavbarUtilities.canApplyCustomPolicy(keyCode) && !isCustomSource) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        if (NavbarUtilities.canApplyCustomPolicy(keyCode) && !isCustomSource) {
            if (mNavbarVisible && !navBarKey /* TODO> && !isADBVirtualKeyOrAnyOtherKeyThatWeNeedToHandleAKAWhenMonkeyTestOrWHATEVER! */) {
                ...
                return 0;
            } else if (!interactive && !PixeldustUtils.isInLockTaskMode()) {
                if (DEBUG_INPUT) {
